...
- [ ] T049 (deferred by user decision 2026-07-11 — run before final submission) Run the availability probe (`backend/scripts/availability-probe.ts`) unattended across a 24-hour window on the demo machine and record the results log as documentation evidence in `docs/testing/` per SC-006 (partial) — **script repaired 2026-08-25 before the run**: since feature 005 wired consent session auth, `POST /api/sessions` rejects the probe's legacy `{ orgId, displayName }` body with 401 `UNAUTHENTICATED` (the legacy path is only enabled by the test harness), so an unattended run would have recorded 0/25; the probe now signs in as its own account (`PROBE_EMAIL`/`PROBE_PASSWORD`, registered on first run) and re-authenticates on a 401. Smoke-verified 3/3 against the live stack with `PROBE_MAX_ATTEMPTS=3`
