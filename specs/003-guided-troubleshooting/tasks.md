...
- [X] T047 Run all five quickstart.md scenarios plus automated gates on the demo machine: `npm --prefix backend run typecheck`, lint, `npm --prefix backend test` (incl. classification regression + guardrail tests), `npm --prefix frontend test` (ChatPage + VoiceControl green) — automated gates green (typecheck + lint clean, backend 426/427 with the sole failure a pre-existing opt-in real-LLM benchmark unrelated to this feature); the 5 manual quickstart.md walkthroughs run live against the demo stack in 006 T019, all PASS, two real defects (OBS-13, OBS-14) found and fixed along the way — see `docs/testing/quickstart-walkthroughs-003.md`
