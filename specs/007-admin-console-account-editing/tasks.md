...
- [X] T055 [P] Revise the profile-editing steps in `docs/testing/uat-scenarios.md` for staff-authoritative editing, and add the SC-008 UAT tasks (one maintainer task and one staff profile-correction task, three testers, unaided, first attempt)
...
**Purpose**: Close the gaps `/speckit-converge` found between the shipped code and this feature's
spec, plan, and the constitution. Appended 2026-08-29; the phases above are unchanged.

- [X] T060 **CRITICAL** Bring `frontend/src/lib/types.ts` back under the 500-line limit per Constitution VI ("Source files ≤ 500 lines; single-responsibility modules") (contradicts) — this feature grew it from 474 to 719 lines at T006. Split the `// --- 007: Maintainer Admin Console & Staff-Authoritative Account Editing` block (line 483 to end, 237 lines) into its own module(s) under `frontend/src/lib/` — the profile provenance/history/directory shapes and the maintainer console view shapes are two independent groups — re-exporting or updating the consumers so `npm run typecheck`, `npm run lint`, and `npm test` stay green in `frontend/` with no behaviour change. Then correct the "500-line rule" row of the T047 record in `docs/testing/observations.md`, which currently reports PASS and names `profile-field-service.ts` at 366 as the largest file this feature touched: the check never covered `types.ts`, so record what was actually missed and the line counts after the split
- [X] T061 Correct every stale maintainer reference in `README.md` per plan T015 and `contracts/api.md` (contradicts) — T015 moved the namespace from `/api/admin` to `/api/maintainer`, so the roles table (:200), the `MAINTAINER_KEY` config row (:268), the seeding note (:281), the "### Maintainer" section heading text (:353) and all six endpoint rows (:357-362), and the Troubleshooting row (:417) currently document paths that 404 in every case — the Troubleshooting row is the worst of them, because it explains a 404 by sending the reader back to the retired namespace. In the same pass fix `:322`, which still describes `POST /staff/users/:id/profile/entries` as appending "a staff note or correction" after T031 retired the correction write path (`CORRECTION_WRITE_RETIRED`, `backend/src/services/profile/profile-service.ts:147`), and `:14` and `:199`, which still frame category administration and staff profile handling as they stood before this feature
- [X] T062 Document this feature's surfaces in `README.md` per Constitution Principle V and SC-002, FR-016, FR-030 (missing) — none of 007 is currently documented. Add: the `/maintainer` console route and what a maintainer does through it rather than by hand-built request (SC-002); `GET /api/maintainer/status` in the Maintainer API table, noting it is unauthenticated and mounted even when `MAINTAINER_KEY` is unset; `GET /staff/accounts` and the three field routes (`PUT /staff/users/:id/profile/fields`, `POST /staff/users/:id/profile/fields/:field/release`, `GET /staff/users/:id/profile/fields/:field/history`) in the Staff API table; `MAINTAINER_SIGNIN_MAX_FAILURES` (5) and `MAINTAINER_SIGNIN_COOLDOWN_SECONDS` (300) in the configuration table with their committed defaults; and How-to-use plus Troubleshooting entries for staff-authoritative editing, per-field locking and release, and the console's three refusal states (wrong key, administration switched off, cooling off)
