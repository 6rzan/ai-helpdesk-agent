...
- **FR-007**: Staff MUST be able to take over any open ticket at any stage — escalation is the usual trigger, not a precondition (Principle III override authority). A ticket is takeover-eligible while it is not resolved or closed; resolved/closed tickets cannot be taken over. Takeover sets the handling mode to human-involved; staff MUST also be able to update ticket status and mark tickets resolved.
- **FR-008**: Every staff action on a ticket (takeover, status change, resolution, profile append) MUST be attributed to the acting staff member with a timestamp. Attribution records are append-only: the system provides no path to edit or delete them (Constitution Principle II audit discipline).
...
- **FR-016**: Staff MUST be able to bulk-import user details from an existing Excel file, choosing during the import which spreadsheet column maps to which account/profile field, with a preview before applying; rejected rows MUST be reported with reasons, and rows matching an existing account (by email) MUST update that profile rather than duplicate it. If the confirmed mapping includes no initial-password column, the system generates an initial password for each created account and includes it in the per-row outcome report.
- **FR-017**: Accounts MUST support provisioned credentials: an account can be created (individually or via import) with an email and a set initial password, and every account holder MUST be able to change their own password in a settings area. Passwords (initial or user-chosen) MUST be at least 8 characters.
...
- **FR-019**: A taken-over ticket MUST be reassignable to another staff member (attributed, with the full assignment history kept on the ticket); tickets are never handed back to automated agent handling once a human is involved. Any staff member may perform a reassignment (attributed to whoever reassigns); an unassigned ticket gains its first assignee only through takeover. After takeover, the agent no longer offers troubleshooting in that conversation: it responds to the reporter only with honest status and handling-mode information, and new reporter messages are preserved on the ticket for the assigned staff member.
...
- Sessions use a rolling expiry (reference: 7 days), suitable for the controlled demo environment. If a session expires or is signed out mid-conversation, the next action redirects to sign-in and the conversation remains intact on the server for resumption. No sign-in lockout or rate limiting is implemented: the system runs in a secured, isolated test environment (NFR-3), and this exclusion is deliberate.
- FR-003 rollout: the anonymous name-entry start is retired. Conversations and tickets created before accounts remain readable and workable under FR-014; there is no retroactive linking of legacy tickets to accounts in this feature.
- "Maintainer" denotes the developer operating the demo machine (console/seed-script actions), not a third in-app role; FR-001's two roles are unaffected.
- Staff members may report issues like any user; a staff member acting on a ticket they reported is permitted, and the action is attributed like any other staff action.
- Accepted exclusions for this feature: account deactivation/offboarding (accounts persist for the project's lifetime), in-system delivery of provisioned initial passwords (handed over out-of-band by staff), and a degraded live-update mode (if the live channel is unavailable, a manual browser refresh is the fallback).
