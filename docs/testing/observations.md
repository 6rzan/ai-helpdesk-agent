...
| 500-line rule | PASS now, after two fixes, and this row is a correction: as first written it reported a clean pass and named `backend/src/services/profile/profile-field-service.ts` at 366 as the largest file this feature touched. That was wrong. `frontend/src/lib/types.ts` was not among the files the T047 check counted, and T006 had grown it from 474 lines to 719, so it stood over the limit for the whole of this feature's build. **Fix 1 (T047)**: `frontend/src/pages/maintainer/CategoryListPage.tsx` had reached 622 lines and was split at the seam between the list and the four screens a view switches to, giving `CategoryListPage.tsx` (274) and the new `CategoryForms.tsx` (372). **Fix 2 (T060, after `/speckit-converge` found the miss)**: `types.ts` was split on a domain seam into `types.ts` (489), `profileTypes.ts` (186) and `maintainerTypes.ts` (91); `types.ts` re-exports the profile shapes, so the ~40 files importing those did not change; the five files using the maintainer console shapes (`CategoryForms.tsx`, `CategoryListPage.tsx`, `GuideEditor.tsx`, `services/maintainerApi.ts`, `tests/pages/CategoryListPage.test.tsx`) had their import path repointed to `./maintainerTypes`, which keeps the console shapes out of the module every account-facing page pulls in. Frontend typecheck, lint, the 284 tests and `npm run build` all stayed green, and no test assertion changed. Largest file this feature touched, after both splits: `frontend/src/lib/types.ts` at 489 |
