...
// Extracted from DashboardPage (T009): the ticket list and its filter controls,
// so the dashboard page has room for the approval queue entry point (T096) and
// the metrics band (T107) without growing past the 500-line ceiling.
...
export interface TicketListProps {
  ...
  sort: NonNullable<StaffTicketFilters["sort"]>;
  onStatusChange: (value: TicketStatus | "") => void;
  onCategoryChange: (value: IssueCategory | "") => void;
  onSortChange: (value: NonNullable<StaffTicketFilters["sort"]>) => void;
  tickets: StaffTicketRow[];
  isLoading: boolean;
  error?: string;
  updatedReferences: Set<string>;
}
    ...
    <>
      <div className="mb-4 flex flex-wrap items-end gap-4">
        ...
        <label className="flex flex-col gap-1 text-xs font-medium text-gray-600">
          ...
          <select
            value={sort}
            onChange={(e) => onSortChange(e.target.value as NonNullable<StaffTicketFilters["sort"]>)}
            className="rounded border border-gray-300 px-2 py-1.5 text-sm text-gray-800"
          >
