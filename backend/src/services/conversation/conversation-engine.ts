...
import type { EscalationReason, InputOrigin, IssueCategory, RefusalReason } from "../../models/enums.js";
...
// The second alternative catches attempted prompt injection ("ignore your
// rules/instructions...") on its own, regardless of phrasing, so an injected
// directive is funnelled through the same audited policy decision as any
// other ad-hoc request rather than silently falling through to a normal
// report (FR-006, quickstart.md's injection row).
const REMEDIATION_PATTERN =
  /\b(can|could|will|would)\s+you\b[^.!?]{0,50}\b(reset|unlock|reinstall|install|uninstall|restart|reboot|delete|remove|wipe|format|change|clear|fix|sort out|handle|take care of|deal with)\b|\bignore\b[^.!?]{0,40}\b(your|the|previous|prior|these)\s+(rules?|instructions?|prompts?)\b/i;
...
export function detectCategoryKeyword(text: string): IssueCategory | null {
...
async function processReply(ctx: ReplyContext): Promise<void> {
  ...
  const resumedTicketIds = await resumeWaitingTickets(ctx);

  if (conversation.pendingAmbiguousRemediation) {
    await handleAmbiguousRemediationReply(ctx, conversation);
    return;
  }
  ...
  if (await tryHandleGuidedReply(ctx, resumedTicketIds)) {
    return;
  }
  ...
  if (scope === "remediation") {
    await handleAdHocRemediationRequest(ctx, conversation);
    return;
  }
...
async function resumeWaitingTickets(ctx: ReplyContext): Promise<Set<string>> {
  ...
  for (const ticket of waiting) {
    ...
    await ticket.save();
    resumedIds.add(ticket._id.toString());
...
// Shared by the explicit "talk to a human" request and, since FR-015, by
// remediation requests too vague or still ambiguous to act on — either way,
// a person needs to see this, whether that means updating the reporter's
// existing ticket or opening one to carry the escalation.
async function escalateForUserRequest(ctx: ReplyContext, reason: EscalationReason = "user_request"): Promise<void> {
  const existing = await Ticket.findOne({ conversationId: ctx.conversationId }).sort({ createdAt: -1 });
  const plainSuffix = reason === "user_request" ? " at your request." : ".";
  ...
  if (existing) {
    ...
    publishEvent(ctx.sessionId, "ticket_updated", {
      ...
      to: "human_involved",
      at: clock.now(),
      plainText: `Ticket ${existing.reference} has been escalated to IT staff${plainSuffix}`,
    });
    return;
  }
...
// FR-016: what used to be one canned line for every remediation-shaped
// request is now a real policy decision — matched against the same exact,
// no-fuzzy-matching engine every proposed action goes through (matchAction,
...
// (US3).
async function handleAdHocRemediationRequest(ctx: ReplyContext, conversation: ConversationDoc): Promise<void> {
...
async function handleAmbiguousRemediationReply(ctx: ReplyContext, conversation: ConversationDoc): Promise<void> {
